...
admin_api = duo_client.Admin(ikey=CISCO_DUO_INTEGRATION_KEY, skey=CISCO_DUO_SECRET_KEY, host=CISCO_DUO_API_HOSTNAME)

sentinel = AzureSentinelConnector(log_analytics_uri=LOG_ANALYTICS_URI, workspace_id=WORKSPACE_ID, shared_key=SHARED_KEY,
                                  log_type=LOG_TYPE, queue_size=5000)

start_ts: int = math.floor(time.time())


def do_activity_log():
    """Run activity log collection"""
    ...
    if check_if_script_runs_too_long(start_ts):
        logging.error('Script is running too long. Saving progress and exit.')
        return


def do_trust_monitor_log():
    """Run TrustMonitor log collection"""
...
def do_authentcation_log():
    """Run user authentication log collection"""
...
def do_administrator_log():
    """Run administrator log collection"""
...
def do_telephony_log():
    """Run telephony log collection"""
...
def do_offline_enrollment_log():
    """Run offline enrollment log collection"""
...
def main(mytimer: func.TimerRequest) -> None:
    ...
    log_types: list[str] = get_log_types()

    dispatcher = dict(activity=do_activity_log,
                      trust_monitor=do_trust_monitor_log,
                      authentication=do_authentcation_log,
                      administrator=do_administrator_log,
                      telephony=do_telephony_log,
                      offline_enrollment=do_offline_enrollment_log)

    for log_type in log_types:
        logging.info('Dispatching log type: %s', log_type)
        dispatcher[log_type]()
